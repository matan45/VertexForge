...
// Approximation of multiple scattering using Hillaire 2020 method
// For each (altitude, sunZenith), integrate scattering over the sphere
// to compute the isotropic 2nd-order scattering contribution
...
void main()
{
    ...
    float altitude = uv.y * (params.atmosphereRadius - params.planetRadius);

    float r = params.planetRadius + altitude;

    // Integrate scattering over hemisphere directions
    const int SAMPLE_COUNT = 64;
    const float INV_SAMPLES = 1.0 / float(SAMPLE_COUNT);

    vec3 lumTotal = vec3(0.0);
    vec3 fms = vec3(0.0); // fraction of scattered light that is multi-scattered

    for (int i = 0; i < SAMPLE_COUNT; ++i)
    {
        // Uniform sphere sampling with Fibonacci spiral
        float phi = 2.0 * PI * fract(float(i) * 0.6180339887); // golden ratio
        float cosTheta = 1.0 - 2.0 * (float(i) + 0.5) * INV_SAMPLES;
        float sinTheta = sqrt(max(1.0 - cosTheta * cosTheta, 0.0));

        vec3 sampleDir = vec3(sinTheta * cos(phi), cosTheta, sinTheta * sin(phi));

        // Ray march from this point in sampleDir direction
        float t0, t1;
        ...
        for (int j = 0; j < MARCH_STEPS; ++j)
        {
            ...
            vec3 sigmaS = params.rayleighScattering.xyz * densityR + params.mieScattering * densityM;
            ...
            vec3 transToSun = sampleTransmittanceLUT(transmittanceLUT,
                params.planetRadius, params.atmosphereRadius, sampleAlt, sunCosZ);

            // Scattering contribution (isotropic phase = 1/(4*PI) for multi-scatter)
            vec3 scatContrib = sigmaS * (transToSun * transmittance) * ds;
            scatterSum += scatContrib;

            // Track multi-scatter fraction
            fms += sigmaS * transmittance * ds;

            transmittance *= sampleTrans;
        }

        // Add ground albedo contribution if we hit the ground
        if (hitGround && tGround0 > 0.0)
        {
            float groundSunCosZ = cosZenith; // approximate
            vec3 transToSun = sampleTransmittanceLUT(transmittanceLUT,
                params.planetRadius, params.atmosphereRadius, 0.0, groundSunCosZ);
    ...
    // Multiple scattering = lumTotal / (1 - fms)
    // This accounts for infinite bounces

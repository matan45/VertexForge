...
namespace services {
    ...
    void PreviewServiceImpl::registerEventHandlers() {
        ...
        if (animationProvider) {
            ...
            dispatcher.registerCommandHandler<events::animpreview::UnloadAnimationPreviewCommand>(
                [this](const events::animpreview::UnloadAnimationPreviewCommand& cmd) {
                    unloadAnimationPreview(cmd.instanceId);
                });
            ...
            dispatcher.registerQueryHandler<events::animpreview::IsAnimationPreviewInitializedQuery>(
                [this](const events::animpreview::IsAnimationPreviewInitializedQuery& query) {
                    return isAnimationPreviewInitialized(query.instanceId);
                });

            dispatcher.registerQueryHandler<events::animpreview::IsAnimationPreviewMeshLoadedQuery>(
                [this](const events::animpreview::IsAnimationPreviewMeshLoadedQuery& query) {
                    return isAnimationPreviewMeshLoaded(query.instanceId);
                });

            dispatcher.registerQueryHandler<events::animpreview::IsAnimationPreviewAnimationLoadedQuery>(
                [this](const events::animpreview::IsAnimationPreviewAnimationLoadedQuery& query) {
                    return isAnimationPreviewAnimationLoaded(query.instanceId);
                });

            dispatcher.registerQueryHandler<events::animpreview::GetAnimationPreviewMeshBoundsQuery>(
                [this](const events::animpreview::GetAnimationPreviewMeshBoundsQuery& query) {
                    return getAnimationPreviewMeshBounds(query.instanceId);
                });
            ...
            dispatcher.registerQueryHandler<events::animpreview::GetAnimationDurationQuery>(
                [this](const events::animpreview::GetAnimationDurationQuery& query) {
                    return getAnimationDuration(query.instanceId);
                });

            dispatcher.registerQueryHandler<events::animpreview::IsAnimationLoopingQuery>(
                [this](const events::animpreview::IsAnimationLoopingQuery& query) {
                    return isAnimationLooping(query.instanceId);
                });

            dispatcher.registerQueryHandler<events::animpreview::GetAnimationPlaybackSpeedQuery>(
                [this](const events::animpreview::GetAnimationPlaybackSpeedQuery& query) {
                    return getAnimationPlaybackSpeed(query.instanceId);
                });
            ...
            dispatcher.registerQueryHandler<events::animpreview::GetAnimationPreviewBoneCountQuery>(
                [this](const events::animpreview::GetAnimationPreviewBoneCountQuery& query) {
                    return getAnimationPreviewBoneCount(query.instanceId);
                });
    ...
    bool PreviewServiceImpl::isAnimationPreviewInitialized(PreviewInstanceId instanceId) const {
        if (animationProvider) {
            return animationProvider->isAnimationPreviewInitialized(instanceId);
        }
        return false;
    }
    ...
    void PreviewServiceImpl::unloadAnimationPreview(PreviewInstanceId instanceId) {
        if (animationProvider) {
            animationProvider->unloadAnimationPreview(instanceId);
        }
    }

    bool PreviewServiceImpl::isAnimationPreviewMeshLoaded(PreviewInstanceId instanceId) const {
        if (animationProvider) {
            return animationProvider->isAnimationPreviewMeshLoaded(instanceId);
        }
        return false;
    }

    bool PreviewServiceImpl::isAnimationPreviewAnimationLoaded(PreviewInstanceId instanceId) const {
        if (animationProvider) {
            return animationProvider->isAnimationPreviewAnimationLoaded(instanceId);
        }
        return false;
    }

    math::AABB PreviewServiceImpl::getAnimationPreviewMeshBounds(PreviewInstanceId instanceId) const {
        if (animationProvider) {
            return animationProvider->getAnimationPreviewMeshBounds(instanceId);
        }
        return math::AABB{};
    }
    ...
    float PreviewServiceImpl::getAnimationDuration(PreviewInstanceId instanceId) const {
        if (animationProvider) {
            return animationProvider->getAnimationDuration(instanceId);
        }
        return 0.0f;
    }
    ...
    bool PreviewServiceImpl::isAnimationLooping(PreviewInstanceId instanceId) const {
        if (animationProvider) {
            return animationProvider->isAnimationLooping(instanceId);
        }
        return true;
    }
    ...
    float PreviewServiceImpl::getAnimationPlaybackSpeed(PreviewInstanceId instanceId) const {
        if (animationProvider) {
            return animationProvider->getAnimationPlaybackSpeed(instanceId);
        }
        return 1.0f;
    }
    ...
    void PreviewServiceImpl::setAnimationPreviewParams(PreviewInstanceId instanceId, const AnimationPreviewParams& params) {
        if (animationProvider) {
    ...
    size_t PreviewServiceImpl::getAnimationPreviewBoneCount(PreviewInstanceId instanceId) const {
        if (animationProvider) {
            return animationProvider->getAnimationPreviewBoneCount(instanceId);
        }
        return 0;
    }

    std::vector<EvaluatedBoneInfo> PreviewServiceImpl::getAnimationPreviewEvaluatedBones(PreviewInstanceId instanceId) const {
        if (animationProvider) {

...
namespace memory {
	...
	struct VramAssetSnapshotData {
		...
		static void publish(VramAssetSnapshotData&& snap) {
			std::lock_guard<std::mutex> lock(mutex());
			storage() = std::move(snap);
		}

		// Consumer side (UI thread): take a copy for this frame's draw.
		static VramAssetSnapshotData read() {
			std::lock_guard<std::mutex> lock(mutex());
			return storage();
		}

	private:
		static VramAssetSnapshotData& storage() {
			static VramAssetSnapshotData snap;
			return snap;
		}
	};

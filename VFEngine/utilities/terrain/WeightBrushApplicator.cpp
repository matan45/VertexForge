...
namespace terrain
{
            ...
            for (uint32_t x = 0; x < params.verticesPerSide; ++x)
            {
                ...
                switch (effectiveType)
                {
                    case PaintBrushType::PaintLayer:
                        paintLayer(weightMap, x, z, params.activeLayer, influence);
                        break;
                    case PaintBrushType::EraseLayer:
                        eraseLayer(weightMap, x, z, params.activeLayer, influence);
                        break;
                    case PaintBrushType::SmoothWeights:
                        smoothWeights(weightMap, x, z, influence);
                        break;
                    case PaintBrushType::FillLayer:
                        fillLayer(weightMap, x, z, params.activeLayer, influence);
                        break;
                }
    ...
    void WeightBrushApplicator::paintLayer(
        ...
        uint32_t layer, float influence)
    {
        ...
        // Decrease other layers proportionally to maintain sum = 1.0
        float otherSum = 0.0f;
        ...
        uint32_t layer, float influence)
    {
        ...
        // Redistribute removed weight to base layer (layer 0)
        uint32_t fallbackLayer = (layer == 0) ? 1 : 0;
        if (fallbackLayer < wm.layerWeights.size())
        {

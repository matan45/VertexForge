...
TEST_CASE("VK-1610 RVT pool geometry: the same budget buys very different page counts")
{
    ...
    const auto plain = terrain::terrainRVTPoolGeometry(128, false);
    ...
    const auto detail = terrain::terrainRVTPoolGeometry(128, true);
    ...
    CHECK(terrain::terrainRVTPoolGeometry(320, true).capacityPages == 1024);
    CHECK(terrain::terrainRVTPoolGeometry(319, true).capacityPages < 1024);
}
...
TEST_CASE("VK-1610 bytesPerTexel is the true sum of the plane formats")
{
    ...
    {
        const auto layout = render::gpudriven::terrainRVTLayout(detail);
        uint32_t summed = 0;
        for (const vk::Format f : layout.planeFormats)
            summed += formatBytes(f);
        CHECK(summed == layout.bytesPerTexel);
        CHECK(summed == terrain::terrainRVTBytesPerTexel(detail));
    }
}
...
TEST_CASE("VK-1610 RVT pool geometry agrees with the graphics-side sizing math")
{
    ...
    {
        const auto layout = render::gpudriven::terrainRVTLayout(detail);
        CHECK(static_cast<uint32_t>(layout.planeFormats.size()) == terrain::terrainRVTPlaneCount(detail));

        // The last two budgets are past VT_MAX_POOL_DIM, so the hardware clamp is exercised on both
        // sides too - the utilities mirror must saturate at the same edge the graphics math does,
        // not keep growing.
        for (const uint32_t budgetMB : {32u, 64u, 128u, 256u, 512u, 1024u, 8192u, 1048576u})
        {
            const auto geo = terrain::terrainRVTPoolGeometry(budgetMB, detail);
...
TEST_CASE("VK-1610 RVT pool geometry clamps like the graphics math does")
{
    ...
    const auto tiny = terrain::terrainRVTPoolGeometry(0, true);
    ...
    const auto huge = terrain::terrainRVTPoolGeometry(4u * 1024u * 1024u, false);

...
TEST_SUITE("ScriptTickGovernorDistance")
{
    TEST_CASE("disabled by default: the multiplier is always 1")
    {
        scripting::ScriptLODConfig cfg; // enabled == false
        CHECK(cfg.enabled == false);
        for (const float d2 : {0.0f, 100.0f, 10000.0f, 1e9f})
            CHECK(scripting::distanceMultiplier(cfg, d2) == doctest::Approx(1.0f));
    }

    TEST_CASE("bucket edges map to the expected multipliers")
    {
        scripting::ScriptLODConfig cfg;
        cfg.enabled = true; // thresholds {25, 75, 150}, multipliers {1, 2, 4, 8}

        CHECK(scripting::distanceMultiplier(cfg, 0.0f) == doctest::Approx(1.0f));
        CHECK(scripting::distanceMultiplier(cfg, 24.0f * 24.0f) == doctest::Approx(1.0f));
        CHECK(scripting::distanceMultiplier(cfg, 26.0f * 26.0f) == doctest::Approx(2.0f));
        CHECK(scripting::distanceMultiplier(cfg, 74.0f * 74.0f) == doctest::Approx(2.0f));
        CHECK(scripting::distanceMultiplier(cfg, 76.0f * 76.0f) == doctest::Approx(4.0f));
        CHECK(scripting::distanceMultiplier(cfg, 149.0f * 149.0f) == doctest::Approx(4.0f));
        CHECK(scripting::distanceMultiplier(cfg, 151.0f * 151.0f) == doctest::Approx(8.0f));
        CHECK(scripting::distanceMultiplier(cfg, 1e6f) == doctest::Approx(8.0f));
    }

    TEST_CASE("distance can only ever slow a script down")
    {
        scripting::ScriptLODConfig cfg;
        cfg.enabled = true;
        cfg.multipliers[0] = 0.25f; // an author trying to speed a script up past its authored rate
        CHECK(scripting::distanceMultiplier(cfg, 0.0f) == doctest::Approx(1.0f));
    }

    TEST_CASE("effectiveDistanceSq IS the significance-score threshold, restated")
    {
        // score = sig/distSq > T  <=>  distSq/sig < 1/T. Bucketing effectiveDistanceSq against d^2
        // is therefore exactly thresholding vfx::significanceScore at T = 1/d^2 — this pins the
        // algebra the design leans on to reuse the VFX semantic without moving that header.
        const float sig = 4.0f;
        const float distSq = 100.0f;
        const float d = 25.0f;

        const float score = sig / distSq;          // vfx::significanceScore
        const float T = 1.0f / (d * d);
        CHECK((score > T) == (scripting::effectiveDistanceSq(distSq, sig) < d * d));
    }

    TEST_CASE("significance scales the effective distance")
    {
        // Twice as significant => scores as if it were closer, so it keeps its rate further out.
        CHECK(scripting::effectiveDistanceSq(100.0f, 1.0f) == doctest::Approx(100.0f));
        CHECK(scripting::effectiveDistanceSq(100.0f, 4.0f) == doctest::Approx(25.0f));
    }

    TEST_CASE("non-finite or zero significance degrades to full rate, never to a stall")
    {
        // Opposite of vfx::significanceScore's NaN->0 ("evict first"): here the cost of over-ticking
        // is a wasted call, but the cost of under-ticking is broken gameplay.
        const float nan = std::numeric_limits<float>::quiet_NaN();
        CHECK(scripting::effectiveDistanceSq(nan, 1.0f) == doctest::Approx(0.0f));
        CHECK(scripting::effectiveDistanceSq(100.0f, nan) == doctest::Approx(0.0f));
        CHECK(scripting::effectiveDistanceSq(std::numeric_limits<float>::infinity(), 1.0f) ==
              doctest::Approx(0.0f));
        // Zero/negative significance must not divide by zero.
        CHECK(std::isfinite(scripting::effectiveDistanceSq(100.0f, 0.0f)));
        CHECK(std::isfinite(scripting::effectiveDistanceSq(100.0f, -5.0f)));
    }

    TEST_CASE("distance multiplies the authored interval (the combination rule)")
    {
        // A script authored at 10Hz, sitting in the furthest bucket, ticks at 0.1 * 8 = 0.8s.
        scripting::ScriptLODConfig cfg;
        cfg.enabled = true;

        const float authored = 0.1f;
        const float effInterval =
            authored * scripting::distanceMultiplier(cfg, scripting::effectiveDistanceSq(200.0f * 200.0f, 1.0f));
        CHECK(effInterval == doctest::Approx(0.8f));

        const auto ticks = runFrames(effInterval, kFrame60, 600); // 10s
        CHECK(ticks.size() >= 12);
        CHECK(ticks.size() <= 14);
    }
}

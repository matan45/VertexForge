#include "RTSpotShadowDenoiser.hpp"
...
{
    RTSpotShadowDenoiser::RTSpotShadowDenoiser(core::Device& device)
        : device(device)
    {
    }

    RTSpotShadowDenoiser::~RTSpotShadowDenoiser()
    {
        cleanup();
    }

    void RTSpotShadowDenoiser::init(uint32_t width, uint32_t height)
    {
        ...
        vfLogInfo("RTSpotShadowDenoiser: Initialized {}x{} x{} slices", width, height, MAX_SLICES);
    }

    void RTSpotShadowDenoiser::cleanup()
    {
        if (!initialized) return;
    ...
    void RTSpotShadowDenoiser::resize(uint32_t width, uint32_t height)
    {
        if (!initialized) return;
        ...
        spatialImagesReady = false;
        for (uint32_t s = 0; s < MAX_SLICES; ++s) sliceHistoryValid[s] = false;
        vfLogInfo("RTSpotShadowDenoiser: Resized to {}x{}", width, height);
    }

    vk::ImageView RTSpotShadowDenoiser::makeLayerView(vk::Image image, vk::Format format, uint32_t layer)
    {
    ...
    void RTSpotShadowDenoiser::createImages(uint32_t w, uint32_t h)
    {
        maskWidth = w;
        maskHeight = h;
    ...
    void RTSpotShadowDenoiser::destroyImages()
    {
        vk::Device vkDevice = device.getLogicalDevice();
    ...
    void RTSpotShadowDenoiser::createSamplers()
    {
        vk::Device vkDevice = device.getLogicalDevice();
    ...
    void RTSpotShadowDenoiser::createDescriptorPools()
    {
        vk::Device vkDevice = device.getLogicalDevice();
    ...
    void RTSpotShadowDenoiser::allocateDescriptorSets()
    {
        vk::Device vkDevice = device.getLogicalDevice();
    ...
    void RTSpotShadowDenoiser::createTemporalPipeline()
    {
        vk::Device vkDevice = device.getLogicalDevice();
        ...
        {
            vfLogError("RTSpotShadowDenoiser: Failed to compile temporal shader: {}", temporalShader->getLastCompilationError());
            return;
        }
    ...
    void RTSpotShadowDenoiser::createSpatialPipeline()
    {
        vk::Device vkDevice = device.getLogicalDevice();
        ...
        {
            vfLogError("RTSpotShadowDenoiser: Failed to compile spatial shader: {}", spatialShader->getLastCompilationError());
            return;
        }
    ...
    void RTSpotShadowDenoiser::createDenoisedMaskSamplerDescriptor()
    {
    ...
    void RTSpotShadowDenoiser::dispatch(vk::CommandBuffer cmd,
                                         ...
                                         uint32_t resourceFrameIndex)
    {
        ...
        bool anyReset = false;
        for (const RTSpotDenoiseInfo& s : slices)
        ...
        for (const RTSpotDenoiseInfo& info : slices)
        {

#include "RTSpotShadowPipeline.hpp"
...
{
    RTSpotShadowPipeline::RTSpotShadowPipeline(core::Device& device)
        : device(device)
    {
    }

    RTSpotShadowPipeline::~RTSpotShadowPipeline()
    {
        cleanup();
    }

    void RTSpotShadowPipeline::init(uint32_t width, uint32_t height, vk::DescriptorSetLayout tlasLayout)
    {
        if (initialized) return;
        if (!device.isRayQuerySupported())
        {
            vfLogWarning("RTSpotShadowPipeline: Ray query not supported");
            return;
        }
        ...
        initialized = true;
        firstFrame = true;
        vfLogInfo("RTSpotShadowPipeline: Initialized {}x{} x{} slices", width, height, MAX_SLICES);
    }

    void RTSpotShadowPipeline::cleanup()
    {
        if (!initialized) return;
    ...
    bool RTSpotShadowPipeline::resize(uint32_t width, uint32_t height)
    {
        ...
        vfLogInfo("RTSpotShadowPipeline: Resized to {}x{}", width, height);
        return true;
    }

    void RTSpotShadowPipeline::createShadowMaskImage(uint32_t w, uint32_t h)
    {
        maskWidth = w;
        maskHeight = h;
    ...
    void RTSpotShadowPipeline::destroyShadowMaskImage()
    {
        vk::Device vkDevice = device.getLogicalDevice();
        for (uint32_t k = 0; k < MAX_SLICES; ++k)
        {
    ...
    void RTSpotShadowPipeline::createSamplers()
    {
        vk::Device vkDevice = device.getLogicalDevice();
    ...
    void RTSpotShadowPipeline::createParamsBuffer()
    {
        paramsBuffer.create(device.getLogicalDevice(), device.getPhysicalDevice(),
                            sizeof(RTShadowParams), device.getMemoryManager());
    }

    void RTSpotShadowPipeline::createDescriptorLayouts(vk::DescriptorSetLayout tlasLayout)
    {
        vk::Device vkDevice = device.getLogicalDevice();
        ...
        // Fragment sampler layout (set 15): sampler2DArray
    ...
    void RTSpotShadowPipeline::createDescriptorPool()
    {
        vk::Device vkDevice = device.getLogicalDevice();
    ...
    void RTSpotShadowPipeline::allocateDescriptorSets()
    {
        vk::Device vkDevice = device.getLogicalDevice();
    ...
    void RTSpotShadowPipeline::createComputePipeline()
    {
        vk::Device vkDevice = device.getLogicalDevice();

        shader = std::make_unique<core::Shader>(device);
        shader->readShader("../../resources/shaders/shadow/rt_shadow_spot.glsl");

        if (shader->getShaderStages().empty())
        {
            vfLogError("RTSpotShadowPipeline: Failed to compile shader: {}", shader->getLastCompilationError());
            return;
        }
        ...
        pushRange.offset = 0;
        pushRange.size = sizeof(RTSpotShadowPushConstants);
    ...
    void RTSpotShadowPipeline::updateOutputDescriptors()
    {
        for (uint32_t k = 0; k < MAX_SLICES; ++k)
        {
    ...
    void RTSpotShadowPipeline::createShadowMaskSamplerDescriptor()
    {
        vk::DescriptorImageInfo maskInfo{};
    ...
    void RTSpotShadowPipeline::dispatch(vk::CommandBuffer cmd,
                                         ...
                                         uint32_t frameIndex)
    {
        ...
        for (const RTSpotDispatchInfo& light : lights)
        {
            ...
            cmd.pushConstants(pipelineLayout, vk::ShaderStageFlagBits::eCompute, 0,
                              sizeof(RTSpotShadowPushConstants), &pc);

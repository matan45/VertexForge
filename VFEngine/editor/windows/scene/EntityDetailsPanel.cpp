...
namespace windows
{
    ...
    void EntityDetailsPanel::drawComponentSection(services::EntityHandle handle)
    {
        ...
        bool hasWaterBody = waterBodyDrawer.draw(handle);                // VK-1607
        ...
        bool hasReflectionProbe = reflectionProbeDrawer.draw(handle);
        ...
        bool hasUITooltip = uiTooltipDrawer.draw(handle);
        bool hasUIWindow = uiWindowDrawer.draw(handle);
        bool hasUIListView = uiListViewDrawer.draw(handle);
        ...
        addComponentPopup.draw({handle, hasCamera, hasMesh, hasAudio2D, hasAudio3D, hasScript,
                                hasCollider, hasRigidBody, hasVehicle, hasPhysicsAnimation, hasVFX, hasVFXSequence,
                                hasBillboard,
                                ...
                                hasDecal, hasReverbZone, hasFogVolume, hasReflectionProbe,
                                hasUIAnimation, hasUIMask,
                                hasUIDraggable, hasUIDropTarget, hasNavInvoker,
                                hasVolumetricNavVolume, hasVolumetricAgent,
                                hasDestructible, hasBuoyancy, hasWaterWakeEmitter, hasWaterBody,

...
namespace plugin
{
    ...
    {
        // Set serialization hooks on SceneSerialization
        serialization::SceneSerialization::setPluginSerializationHooks(
            // Serialize hook — auto-serialize from property descriptors
            [](scene::Entity& entity) -> nlohmann::json
            {
                ...
                {
                    // Data is already JSON — write it directly with plugin: prefix
                    result["plugin:" + qualifiedName] = data;
                }
                return result;
            },
            // Deserialize hook — auto-deserialize with type validation from descriptors
            [](const nlohmann::json& pluginEntries, scene::Entity& entity)
            {
                ...
                {
                    // Strip "plugin:" prefix (7 chars)
                    std::string qualifiedName = key.substr(7);
                    const auto* info = registry.findComponent(qualifiedName);

                    if (info)
                    {
                        // Merge loaded data with defaults (handles new properties added after save)
                    ...
                    {
                        // Plugin not loaded, store raw JSON for potential future loading
